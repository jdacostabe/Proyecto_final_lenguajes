...
public class VisitorDuplicatedCodeIfLinesInitials<T> extends Python3BaseVisitor<T> {
    String final_text;
    String line_text;
    int line;
    String indent;
    int delete_blocks;
    int convert_else;
    public VisitorDuplicatedCodeIfLinesInitials(ParseTree tree){
        this.final_text = "";
        this.line = 1;
        this.line_text="";
        this.indent="";
    ...
    @Override public T visitIf_stmt(Python3Parser.If_stmtContext ctx) {
        //CONTANDO LINEAS INICIALES COMUNES
        int same_initial_lines = 0;

        for(int num_actual_line=0; num_actual_line<ctx.suite(0).stmt().size(); num_actual_line++){
            String actual_line = ctx.suite(0).stmt(num_actual_line).getText().trim();

            boolean same_actual_line = true;
            for(int num_suite = 1; num_suite<ctx.suite().size(); num_suite++){

                if(ctx.suite(num_suite).stmt(num_actual_line) == null){
                    same_actual_line = false;
                    break;
                }

                if(!ctx.suite(num_suite).stmt(num_actual_line).getText().trim().equals(actual_line)){
                    same_actual_line = false;
                    break;
                }
            }
        ...
        this.delete_blocks++;
        for(int i =0; i<same_initial_lines; i++){
        ...
        int same_code_block = 0;
        if(ctx.suite(0).stmt().size() - same_initial_lines > 0){
            print_block_if = true;
            print_terminal((TerminalNodeImpl) ctx.IF());
            print_code(ctx.test(0));

            print_terminal((TerminalNodeImpl) ctx.COLON(0));
            line = ((TerminalNodeImpl) ctx.IF()).symbol.getLine() + same_initial_lines;
            for(int i=same_initial_lines; i<ctx.suite(0).stmt().size(); i++)
                visit(ctx.suite(0).stmt(i));
        }
        else {
        ...
        //IMPRIMIENDO BLOQUES ElIF - SIN LINEAS INICIALES COMUNES
        for(int num_elif=0; num_elif<ctx.ELIF().size(); num_elif++){
            if(ctx.suite(num_elif+1).stmt().size() - same_initial_lines > 0){
                ...
                line = ((TerminalNodeImpl) ctx.ELIF(num_elif)).symbol.getLine() + same_initial_lines;
                for(int i=same_initial_lines; i<ctx.suite(num_elif+1).stmt().size(); i++)
                    visit(ctx.suite(num_elif+1).stmt(i));
            }
            else {
                line = ((TerminalNodeImpl) ctx.ELIF(num_elif)).symbol.getLine() + same_initial_lines + 2;
            }
        }

        //IMPRIMENDO BLOQUE ELSE - SIN LINEAS INICIALES COMUNES
        if((ctx.ELSE() != null) && (ctx.suite(ctx.ELIF().size()+1).stmt().size()-same_initial_lines >0)){
            if(!print_block_if){
                print_terminal((TerminalNodeImpl) ctx.IF());
            ...
            line = ((TerminalNodeImpl) ctx.ELSE()).symbol.getLine() + same_initial_lines;
            for(int i=same_initial_lines; i<ctx.suite(ctx.ELIF().size()+1).stmt().size(); i++)
